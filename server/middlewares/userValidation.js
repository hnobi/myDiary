  ...
  static signUp(req, res, next) {
    ...
    if (!validator.isEmpty(fullname)) {
      for (let i = 0; i < fullname.length; i += 1) {
        ...
        if (!validator.isLength(fullname, { min: 5, max: 255 })) {
          errors.fullname = 'Fullname  must be between 5 to 150 characters';
        }
      }
    ...
    if (!validator.isEmpty(username)) {
      if (!validator.isLength(username, { min: 2, max: 100 })) {
        errors.username = 'username must be between 5 to 100 characters';
      }
